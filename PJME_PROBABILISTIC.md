...
- Static calibration isolation: global and hourly corrections use validation
  only. The later rolling experiment is explicitly prequential.
...
first holdout week uses validation only; each subsequent week may use labels
from earlier completed holdout weeks, but never from itself or the future. Aggregate
...
- The interval has marginal coverage across the full holdout year; it does not
...
- The rolling result is prequential rather than static holdout evaluation
  because labels from completed holdout weeks calibrate later weeks.
...
explicit minimum sample sizes and no holdout-based tuning.
